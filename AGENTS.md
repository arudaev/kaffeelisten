...
Context file for Codex. Read this before touching any file.
...
`.env` lives at the repo root and is gitignored. It holds all real secrets and tokens used locally and by Codex agents. `.env.example` (committed) shows the required keys with placeholder values.

### For a teammate's Codex agent

When a teammate's Codex session starts on a fresh clone:

1. Copy `.env.example` to `.env` and fill in the real values (share via a secure channel — never commit them).
2. The agent reads `.env` via the shell automatically — no extra setup needed for Codex.
